...
# 入力と出力を指定
photo_size = 200
im_rows = 200 # 画像の縦ピクセルサイズ
im_cols = 200 # 画像の横ピクセルサイズ
im_color = 3 # 画像の色空間
...
# モデルを保存するファイルパス
filepath = './model/cifar10_ResNet50_finetuning_model_holdout.h5'
...
model = Model(resnet50_model.input, top_model(resnet50_model.output))
...
def save_model(model):
    ...
    if not os.path.isdir("cache"):
        os.mkdir("cache")
    json_name = "architecture_resnet50.json"
...
# ファイルのエクスポート
sub.to_csv('./submit/cifar10_ResNet50_finetuning_model_holdout.tsv', sep='\t', header=None, index=None)