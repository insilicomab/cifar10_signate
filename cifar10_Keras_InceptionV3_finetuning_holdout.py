...
# モデルを保存するファイルパス
filepath = './model/cifar10_InceptionV3_finetuning_model_holdout.h5'
    
# ResNet50モデルと学習済みの重みをロード（全結合層は除く）
input_tensor = Input(shape=(im_cols, im_rows, im_color))
inceptionV3_model = InceptionV3(include_top=False, weights='imagenet', input_tensor=input_tensor)
...
model = Model(inceptionV3_model.input, top_model(inceptionV3_model.output))
...
def save_model(model):
    ...
    if not os.path.isdir("cache"):
        os.mkdir("cache")
    json_name = "architecture_InceptionV3.json"
...
# ファイルのエクスポート
sub.to_csv('./submit/cifar10_InceptionV3_finetuning_model_holdout.tsv', sep='\t', header=None, index=None)