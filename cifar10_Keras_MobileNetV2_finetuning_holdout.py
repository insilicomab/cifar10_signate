...
# モデルを保存するファイルパス
filepath = './model/cifar10_MobileNetV2_finetuning_model_holdout.h5'
    
# ResNet50モデルと学習済みの重みをロード（全結合層は除く）
input_tensor = Input(shape=(im_cols, im_rows, im_color))
mobileNetV2_model = MobileNetV2(include_top=False, weights='imagenet', input_tensor=input_tensor)

# 全結合層の構築
top_model = Sequential()
top_model.add(Flatten(input_shape=mobileNetV2_model.output_shape[1:]))
...
model = Model(mobileNetV2_model.input, top_model(mobileNetV2_model.output))
...
def save_model(model):
    ...
    if not os.path.isdir("cache"):
        os.mkdir("cache")
    json_name = "architecture_MobileNetV2.json"
...
# ファイルのエクスポート
sub.to_csv('./submit/cifar10_MobileNetV2_finetuning_model_holdout.tsv', sep='\t', header=None, index=None)