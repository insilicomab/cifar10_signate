...
for train_index, val_index in kf.split(X_train):
    ...
    # モデルを保存するファイルパス
    filepath = './model/cifar10_MobileNet_finetuning_Kfold[%d].h5' % index
    index += 1
...
def save_model(model):
    ...
    if not os.path.isdir("cache"):
        os.mkdir("cache")
    json_name = "architecture_MobileNet_Kfold.json"
...
# ファイルのエクスポート
sub.to_csv('./submit/cifar10_MobileNet_finetuning_Kfold.tsv', sep='\t', header=None, index=None)